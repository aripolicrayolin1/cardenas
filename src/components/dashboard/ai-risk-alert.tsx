...
export function AIRiskAlert({ sensorValues }: AIRiskAlertProps) {
  ...
  const handleManualScan = async () => {
    ...
    try {
      const result = await predictivePestDiseaseAlerts({
        ...
        cropType: "Maíz/Hortalizas",
        region: "Valle del Mezquital, Hidalgo"
      });
      setPrediction(result);
    } catch (error) {
      console.error("Error en IA Predictiva:", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Card className={`glass-card border-none shadow-xl transition-all duration-700 relative overflow-hidden group ${prediction?.predictedRisk === 'High' ? 'ring-2 ring-destructive/50' : 'ring-1 ring-primary/20'}`}>
      {loading && (
        <div className="absolute inset-0 bg-primary/10 z-20 pointer-events-none">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-white/60 backdrop-blur-md">
            ...
            <p className="text-sm font-black uppercase tracking-widest text-primary animate-bounce">
              PROCESANDO DATOS SENSORIALES CON IA...
            </p>
          </div>
        </div>
      )}
        ...
        {!prediction && !loading ? (
          <div className="py-8 text-center">
            ...
            <p className="text-xs font-bold text-muted-foreground mb-6 uppercase tracking-tighter">
              Inicia el análisis de riesgo basado en sensores mediante IA real.
            </p>
            <Button onClick={handleManualScan} className="gap-2 font-black rounded-xl px-10 py-6 bg-primary hover:bg-primary/90 shadow-lg uppercase tracking-widest">
               <Zap className="h-5 w-5 fill-white" /> ANALIZAR CON IA
            </Button>
          </div>
        ) : prediction ? (
          <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <Button variant="ghost" size="sm" onClick={handleManualScan} className="w-full text-[9px] h-8 font-black uppercase tracking-widest text-muted-foreground hover:text-primary rounded-xl">
              <RefreshCw className="h-3.5 w-3.5 mr-2" /> RE-CALCULAR RIESGO (IA)
            </Button>
          </div>
        ) : null}
      </CardContent>
